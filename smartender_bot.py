...
class SmartenderBot:
    def __init__(self, token, smartender):
        ...
        Args:
            ...
            smartender (Smartender): Reference to the Smartender instance
        """
        self.token = token
    ...
    def handle_callback_query(self, msg):
        ...
        print(f"Received callback query from {msg['from']['username']}, chat {chat_id}: {query_data}")

        # Make the selected cocktail
        self.bot.answerCallbackQuery(query_id, text=f"Starting to prepare {query_data}!")
        self.bot.sendMessage(chat_id, f"Preparing {query_data}... Please wait!")

        try:
            self.smartender.make_cocktail(query_data)
    ...
    def send_cocktail_menu(self, chat_id, username):
        ...
        try:
            with open('selected_cocktails.json', 'r') as file:
                data = json.load(file)
                cocktails = data['available_cocktails']
